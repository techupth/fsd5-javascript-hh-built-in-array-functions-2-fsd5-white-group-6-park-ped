...
const filterLocal = bills.map((justLocal) => {
  return justLocal.location;
});

const uniqueLocal = filterLocal.filter((name, index) => {
  return filterLocal.indexOf(name) === index;
});
function sumTotalUniqueLocation() {
  for (let i of uniqueLocal) {
    objects[i] = 0;
  }
  for (let i of bills) {
    //วิธีการเขียนแบบที่ 1 เข้าใจง่ายเหมาะกับมือใหม่
    //   for (let j in objects) {
    //     if (i.location === j) {
    //       objects[i.location] += Number(i.total);
    //     }
    //   }
    //--------
    //วิธีการเขียยนแบบที่ 2 จะต้องรู้กระบวนการทำงานภายใน function built-in
    if (Object.keys(objects).includes(i.location)) {
      objects[i.location] += Number(i.total);
    }

    //ทั้ง 2 วิธีได้ผลลัพธ์แบบเดียวกัน
  }
  return objects;
}
let objects = {};
const totalPaidByLocation = sumTotalUniqueLocation();
